...
- 已有 HeroUI v3 + Tailwind CSS v4 工作台 UI、Onboarding、Jobs、Profile、Resumes 的页面骨架和本地开发 fixture。
...
- 已有职位表浏览、admin 职位导入 UI 与 `job-parse` Edge Function（Dify vision 解析 JD 文本/截图）。
- 规则匹配分、AI 叙事分析、target resume 生成仍待后续任务接入。
...
```bash
DIFY_RESUME_PARSE_API_KEY=   # 简历附件解析 workflow
...
7. 在 Resumes 页面验证上传按钮、格式校验和错误提示。
8. Admin 账号：在 `/jobs` 导入职位（粘贴 JD 或上传截图解析预填），编辑/停用职位；普通用户无导入入口。
9. 后续阶段再接入规则匹配分、AI 叙事分析、target resume 生成和编辑器。
