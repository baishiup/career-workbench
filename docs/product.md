...
说明：不需要独立的面试官、访客或公开体验角色。mock 数据只是开发阶段的 fixture 和降级验证手段，不作为产品角色或后续主流程。
...
3. 用户查看 Jobs 列表和职位详情。
4. 系统按代码规则实时计算职位匹配分；用户在职位详情按需触发 AI 叙事分析，输出证据、缺口和风险表达。
...
- 匹配分数由代码规则基于 Profile 和结构化 JD 实时计算，不持久化；`MatchReport` 只保存按需生成的 AI 叙事（证据、缺口、风险），可缓存复用，并按 Profile / 职位更新时间判断过期。
...
flowchart TD
  ...
  HasProfile -- "是" --> Jobs
  Jobs --> JobDetail["职位详情（规则匹配分）"]
  JobDetail --> Match["按需触发 AI 叙事分析"]
  Match --> Report["查看匹配报告"]
...
- Supabase Auth、Profile 持久化和核心业务表的接入。
- Dify Workflow / Chatflow 的解析、匹配和生成链路接入。
